...
`schemaVersion=4` и `contentVersion=browser-content-2026.08-master.2` сохраняются внутри `GameState`. При чтении local save browser edition:

| Состояние | Поведение |
|---|---|
| Legacy schema 1–3 | Мигрируется в schema 4, отмечается Archive trace. |
...
| Отсутствующие canonical tranche fields | Восстанавливаются safe defaults: deterministic seed, dormant event, начальный tutorial, пустой recognition ledger и отключённый local feedback. |
